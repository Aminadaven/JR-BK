...
import com.aminadav_studio.jr.bk.Interfaces.*;
...
class Hold
{
	...
	void recRU(int sum) {
		...
		ru += sum;
		//Main.ui.news.append(owner.toString() + " Has Recruited " + sum + " Ranged Units.\n");
	}
	...
	void buildEconomy() {
		...
		owner.money -= ecoCost();
		economy++;
		//Main.ui.news.append(owner.toString() + " Has Built up his Economy to level:"	+ economy + "\n");
	}
...
final class Judea extends Player
{
	public Judea()
	{
		super(3500, 0.7, 6, 9);//first is 4000
	}
	...
	void endTurn() {
		super.endTurn();
		pHold.pListen.update(Main.ui.selected.pop, getPop());
		pHold.incListen.update(Main.ui.selected.calcIncome());
	}
}

class pHold extends Hold
{
	static IText nListen;
	static LPop pListen;
	static LUnit muListen, ruListen;
	static IDouble hapListen;
	static IBoolean barListen, rngListen;
	static IInt incListen, ecoListen, mListen;
/*
	...
	*/
	...
	void recMU(int sum) {
		super.recMU(sum);
		nListen
			.update(owner.toString() + " Has Recruited " + sum + " Melee Units.\n");
		muListen.update(mu);
		pListen.update(pop, ((Judea) owner).getPop());
		incListen.update(calcIncome());
	...
	void recRU(int sum) {
		super.recRU(sum);
		nListen
			.update(owner.toString() + " Has Recruited " + sum + " Ranged Units.\n");
		ruListen.update(ru);
		pListen.update(pop, ((Judea) owner).getPop());
		incListen.update(calcIncome());
		mListen.update(owner.money);
	...
	void buildBar() {
		super.buildBar();
		nListen.update(owner.toString() + " Has Built Barracks!\n");
	...
	void buildRange() {
		super.buildRange();
		nListen.update(owner.toString() + " Has Built Range!\n");
	...
	void buildEconomy() {
		super.buildEconomy();
		nListen.update(owner.toString() + " Has Built up his Economy to level:"
			+ economy + "\n");
	...
	void raiseHappiness() {
		...
		nListen.update("");
		hapListen.update(happiness, hapCost());
		incListen.update(calcIncome());
		mListen.update(owner.money);
	}
	...
	void attack(Hold hold, int mu, int ru) {
		if (hold.owner.equals(owner) || mu > this.mu || ru > this.ru) {
			nListen.update(owner.toString() + " Can't Attack!\n");
			return;
		}
		if (mu + ru <= 0) {
			nListen.update(owner.toString() + " 0 - Soldiers!\n");
			return;
		}
		nListen.update(owner.toString() + " is Attacking!\n");
		Main.Battle(hold, this, mu, ru);
		muListen.update(mu);
		ruListen.update(ru);
		incListen.update(calcIncome());
...
final class Rome extends Player
{
	...
	{
		super(1000, 0.5, 8, 7);//first is 1,000,000
